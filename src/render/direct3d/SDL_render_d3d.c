...
// ADDED BY LWEISE FOR CREX
// added return int
static int D3D_RenderPresent(SDL_Renderer * renderer, int wait, int intervalOne);
////////////////////////////////////////////////////////////
...
static int
D3D_RenderPresent(SDL_Renderer * renderer, int wait, int intervalOne)
{
    D3D_RenderData *data = (D3D_RenderData *) renderer->driverdata;
    HRESULT result;
	...
	//////////////////////////////////////////////////////////////////////////////////////
	// ADDED BY LWEISE for CREx
	D3DRASTER_STATUS tempstatus;
	IDirect3DSwapChain9* tempchain;
	data->device->lpVtbl->GetSwapChain(data->device, 0, &tempchain);
	// wait until VB, then try presenting until end of VB
	HRESULT presout;
	int breakout = 0;

	if (!intervalOne) {
		while (!breakout) {
			data->device->lpVtbl->GetRasterStatus(data->device, 0, &tempstatus);
			// while ( tempstatus.InVBlank) { data->device->lpVtbl->GetRasterStatus(data->device, 0, &tempstatus);	}
			while (!tempstatus.InVBlank) { data->device->lpVtbl->GetRasterStatus(data->device, 0, &tempstatus); }

			while (!breakout && tempstatus.InVBlank) {
				// then keep trying to schedule present, until either success or end of VBinterval
				presout = tempchain->lpVtbl->Present(tempchain, NULL, NULL, NULL, NULL, D3DPRESENT_DONOTWAIT);
				data->device->lpVtbl->GetRasterStatus(data->device, 0, &tempstatus);
				if (presout != D3DERR_WASSTILLDRAWING)
					breakout = 1;
			}
		}
	} else if (intervalOne) {
    ...
    //}
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	// ADDED BY LWEISE FOR CREx PACKAGE
	// after present returns, we wait until the next screen-refresh is finished
	//IDirect3DSwapChain9 tempchain;
	//int chaincount = 0;
	//while (data->device->lpVtbl->GetSwapChain(data->device, chaincount, &tempchain) != D3DERR_INVALIDCALL) {
	//	++chaincount;
	//}
	//HRESULT tempres = data->device->lpVtbl->GetSwapChain(data->device, 0, &tempchain);
	//HRESULT tempres = IDirect3DDevice9_GetRasterStatus(data->device, 0, &tempstatus);
	//int return_nvb = 0;
	int one   = 0;
	int two   = 0;
	int timeref1 = 0;
	...
	if (wait) {
		// present done when successfully scheduled
		// thus, we wait out the remaining VB
		// then we wait the following retrace, followed by another VB and retrace
		data->device->lpVtbl->GetRasterStatus(data->device, 0, &tempstatus);

		while (tempstatus.InVBlank) {
			++one;
			data->device->lpVtbl->GetRasterStatus(data->device, 0, &tempstatus);
		}
		timeref1 = timeGetTime();
		while (!tempstatus.InVBlank) {
			++two;
			data->device->lpVtbl->GetRasterStatus(data->device, 0, &tempstatus);
		}
		timeref2 = timeGetTime();
	}
	//return ( ((one>0)+1)*1000 + ((two>0) + 1)*100 + ((three>0) + 1)*10 + ((four>0) + 1)*1 );   // return number for debugging: which waits actually happend?
	switch (wait) {
		case 1: return timeref1 + (timeref2 - timeref1) / 2; break;
